...
def process_brush_data(df, dna_concentration_uM=5.0, lipid_volume_uL=5.0, 
                      lipid_concentration_mg_mL=0.5, lipid_molar_mass=760,
                      control_name="ctrl"):
    """
    ...
    df_data["dna_moles"] = df_data["sample_number_clean"] * 1e-6 * dna_concentration_uM * 1e-6
...
def process_charged_liposome_data(df):
    ...
    for condition in df['condition'].unique():
        ...
        for batch in samples['batch'].unique():
            ...
            if len(batch_controls) > 0:
                ...
                batch_samples.loc[:, 'delta_z_average'] = batch_samples['z-average'] - control_z_avg
                batch_samples.loc[:, 'delta_p1mi'] = batch_samples['p1mi'] - control_p1mi
    ...
    df_processed['DOPG_percentage'] = df_processed['condition'].map(condition_to_dopg)
