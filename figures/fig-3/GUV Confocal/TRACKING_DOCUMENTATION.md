...
2. Detects vesicles by finding dark regions in the fluorescence channel
3. Links detections across frames using trackpy
4. Filters tracks based on quality criteria
5. Measures membrane fluorescence in an annulus around each vesicle
...
```
Channel 0 = Fluorescence (used for detection AND measurement)
Channel 1 = Brightfield (currently unused, loaded but not processed)
```

The fluorescence channel contains a membrane-bound dye. Vesicle interiors appear **dark** because they exclude the dye.
...
TrackingConfig(
    ...
    annulus_width_px=3,
    dark_threshold_factor=0.7,  # Pixels < 0.7 × Otsu threshold are "dark"
    max_eccentricity=0.7,       # Allow elliptical (0=circle, 1=line)
