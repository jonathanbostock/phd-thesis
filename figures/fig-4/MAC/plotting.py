...
def main():
    ...
    initial_df_full = convert_csv_to_df(os.path.join(file_path, "initial.csv"))
    timecourse_df_full = convert_csv_to_df(os.path.join(file_path, "c9-timecourse.csv"))
    triton_df_full = convert_csv_to_df(os.path.join(file_path, "triton.csv")).iloc[:1]

    # Select columns - ensure we get DataFrames
    initial_df = pd.DataFrame(initial_df_full[columns])
    timecourse_df = pd.DataFrame(timecourse_df_full[columns])
    triton_df: pd.DataFrame = pd.DataFrame(triton_df_full[columns])
