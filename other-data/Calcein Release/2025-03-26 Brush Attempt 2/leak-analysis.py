...
def main():
    ...
    jbplot.plotdf(ax, df_processed,
                  ...
                  split="Sample Type",
                  plot_type="line",
                  name_map= lambda tuple: tuple[0].replace("_", " ").title())
    ...
    fig, ax = jbplot.figax()
    jbplot.plotdf(ax, df_processed.loc[df_processed["Time / min"] < 10.5],
                  x="Time / min",
                  y="Mean",
                  y_sig="SEM",
                  split="Sample Type",
    ...
    jbplot.save(fig, Path(__file__).parent / "Calcein Leakage Ten Minutes")
