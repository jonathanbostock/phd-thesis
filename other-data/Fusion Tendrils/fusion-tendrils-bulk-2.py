...
before_df = (pd.read_csv("2023-08-15 Bulk Tendrils Before 1000V.csv",
                        header=1,
                        skiprows = lambda x: x>3)
             .drop(columns=["Time (min).{}".format(n) for n in range(1,36)]))

tc_df = (pd.read_csv("2023-08-15 Bulk Tendrils Timecourse 1000V.csv",
                    header=1,
                    skiprows = lambda x: x>92)
         .drop(columns=["Time (min).{}".format(n) for n in range(1,36)]))


##Make some initial lists
condition_list = ["Long +IM","Long -IM","Short +IM","Short -IM"]
...
factor_dict = {}
factor_dict["Long Tendrils"] = np.average(proc_df.loc[58:63,"Long -IM"])/np.average(proc_df.loc[58:63,"Long +IM"])
factor_dict["Truncated Tendrils"] = np.average(proc_df.loc[58:63,"Short -IM"])/np.average(proc_df.loc[58:63,"Short +IM"])
